...
// This replaces the previous hand-rolled GF(256) implementation with a
// battle-tested, SIMD-accelerated library that supports AVX2/NEON.
...
// This is more memory-efficient for large data as it processes in chunks.
...
func (ec *ECEncoder) Decode(shards [][]byte, shardPresent []bool, originalLen int) ([]byte, error) {
	...
	for i := 0; i < k+m; i++ {
		...
		// Missing shards remain nil — reedsolomon.Reconstruct handles this
	}
