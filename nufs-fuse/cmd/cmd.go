...
func NewApp() *cli.App {
	...
	app.Action = func(c *cli.Context) error {
		...
		for _, option := range strings.Split(c.String("o"), ",") {
			...
			case "read-only":
				opts = append(opts, minfs.ReadOnly())
			case "scan-ttl":
				if len(vals) == 1 {
					return errors.New("scan-ttl has no value")
				}
				ttl, err := time.ParseDuration(vals[1])
				if err != nil {
					return fmt.Errorf("scan-ttl is not a valid duration: %s", vals[1])
				}
				opts = append(opts, minfs.ScanTTL(ttl))
			case "metrics":
				addr := ":9900"
				if len(vals) > 1 {
					addr = vals[1]
				}
				opts = append(opts, minfs.MetricsAddr(addr))
			default:
				// Silently skip unknown fuse-style options (e.g. allow_other)
				// to maintain compatibility with standard mount(8) syntax.
			}
		}

		// Target and mountpoint are positional args, set once outside the option loop.
		target := c.Args().Get(0)
